...
* **Dashboard Trực quan & Thống kê Toàn quyền:**
  ...
  * Xem **Tổng Doanh Thu** toàn bộ lịch sử kinh doanh.
...
* **Trích xuất Dữ liệu (Export):** Cho phép xuất hóa đơn mua hàng và lưu các loại báo cáo doanh thu ra định dạng **PDF**.
* **Biểu đồ Trực quan:** Ứng dụng JFreeChart để vẽ trực tiếp biểu đồ minh họa Top Doanh Thu cực kỳ trực quan.
* **Sao Lưu Cơ Sở Dữ Liệu (Database Backup):** Tích hợp công cụ hỗ trợ cho Admin phân xuất (dump) toàn bộ dữ liệu bán hàng. Cơ chế thông minh có khả năng tự động rà quét và dò tìm vị trí cài đặt MySQL/XAMPP trên máy tính để thực thi mà không cần cấu hình PATH phức tạp.
---
