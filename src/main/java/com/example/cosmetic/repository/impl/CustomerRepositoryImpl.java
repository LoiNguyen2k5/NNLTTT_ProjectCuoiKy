...
public class CustomerRepositoryImpl implements CustomerRepository {
    ...
    public void save(Customer c) {
        ...
        try { tx.begin(); em.persist(c); tx.commit(); } 
    ...
    public void update(Customer c) {
        EntityManager em = JpaUtil.getEntityManager();
        EntityTransaction tx = em.getTransaction();
        try { tx.begin(); em.merge(c); tx.commit(); } 
        catch (Exception e) { tx.rollback(); throw e; } 
        finally { em.close(); }
    }
    ...
    public List<Customer> searchByPhone(String phone) {
        EntityManager em = JpaUtil.getEntityManager();
        try {
            return em.createQuery("SELECT c FROM Customer c WHERE c.phoneNumber LIKE :phone", Customer.class)
